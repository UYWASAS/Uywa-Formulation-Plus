...
def _create_project_zip_export(
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr("requirements.csv", pd.DataFrame(req_list).to_csv(index=False))
...
def _render_progress(min_val, max_val, obtained):
    ...
    if min_val > 0:
        ...
        pct = (obtained / max_val) * 100 if max_val else 0
    else:
        pct = 100
...
def render():
    ...
    # ----------------------------
    # 1) Carga/restore de proyecto ZIP
    # ----------------------------
    with st.expander("Cargar proyecto completo UYWA (.zip)", expanded=False):
        ...
        if up_zip is not None and st.button("Restaurar proyecto", key="aves_restore_zip_btn"):
            ...
            if errors:
                ...
                st.session_state["aves_max_limits_loaded"] = max_l
    ...
    # ----------------------------
    # 3) Selección ingredientes + edición
    # ----------------------------
    ...
    with st.expander("Descargar o cargar matriz seleccionada", expanded=False):
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Preparar descarga CSV", key="aves_download_matrix_prepare"):
                st.download_button(
                    "Descargar matriz seleccionada",
                    data=_create_ingredients_csv(df_sel),
                    file_name=f"matriz_aves_{date.today().strftime('%Y%m%d')}.csv",
                    mime="text/csv",
                    key="aves_download_matrix_btn",
                )
        with c2:
            up_sel = st.file_uploader("Cargar matriz seleccionada (CSV)", type=["csv"], key="aves_upload_selected_csv")
            if up_sel is not None and st.button("Aplicar matriz cargada", key="aves_apply_selected_csv"):
                try:
                    dtmp = pd.read_csv(up_sel)
                    if "Ingrediente" not in dtmp.columns:
                        st.error("El CSV debe contener columna 'Ingrediente'.")
                    else:
                        ing_valid = [x for x in dtmp["Ingrediente"].astype(str).tolist() if x in ing_all]
                        st.session_state["aves_ingredientes_sel"] = ing_valid
                        st.success(f"Se cargaron {len(ing_valid)} ingredientes.")
                        st.rerun()
    ...
    if ing_limit:
        with st.expander("Editar límites por ingrediente", expanded=True):
            ...
            for ing in ing_limit:
                ...
                default_max = float(st.session_state.get(f"aves_max_{ing}", max_loaded.get(ing, 100.0)))
                default_min = st.session_state.get(f"aves_min_{ing}", min_loaded.get(ing, ""))
    ...
    ratios_active = [
        r for r in st.session_state["aves_ratios"]
    ...
    if df_sel is not None and not df_sel.empty and req_input:
        pname = st.text_input(
            "Nombre del proyecto",
            value=f"Aves_{etapa}_{date.today().strftime('%Y%m%d')}",
        ...
        if st.button("Preparar ZIP", key="aves_prepare_zip"):
            ...
            st.download_button(
                ...
                key="aves_download_zip",
            )

    # ----------------------------
    # 9) Validación + precheck + solver final
    # ----------------------------
    st.markdown("---")
    ...
    # ----------------------------
    # 10) Vista previa del último resultado
    # ----------------------------
    ...
    if result:
        ...
        if result.get("success"):
            ...
            if not df_d.empty:
                ...
                render_table(
                    df_d,
                    column_config={"Inclusión (%)": st.column_config.NumberColumn("Inclusión (%)", format="%.4f")},
                )
        else:
